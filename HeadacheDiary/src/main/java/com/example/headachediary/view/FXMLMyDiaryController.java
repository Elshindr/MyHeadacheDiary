...
public class FXMLMyDiaryController {
    ...
    private String[] arrTrig = {"Light", "Sound", "Smell", "Movement", "Sleep", "Lake of Sleep", "Period", "Coffee", "Tea", "Other"};
    private String[] arrSens = {"Light", "Sound", "Sleep", "Movement","Smell"};
    private String[] arrSympKey = {"Severity", "Dizziness", "Aura", "Nausea", "Side"};
    private String[] arrSympValue ={"None", "Mild", "Moderate", "Severe", "Vertigo", "Vomiting", "Left", "Right", "Both"};
    private String[] arrMed = {"Ibuprofen","Paracetamol","Triptan","Other"};
    ...
    private ObservableList<String> obsDiz = FXCollections.observableArrayList(arrSympValue[0], arrSympValue[1], arrSympValue[2], arrSympValue[4]);
    private ObservableList<String> obsNausea = FXCollections.observableArrayList(arrSympValue[0], arrSympValue[1], arrSympValue[2], arrSympValue[5]);
    private ObservableList<String> obsAura = FXCollections.observableArrayList(arrSympValue[0], arrSympValue[1], arrSympValue[2]);
    private ObservableList<String> obsSevere = FXCollections.observableArrayList(arrSympValue[1], arrSympValue[2], arrSympValue[3]);
    private ObservableList<String> obsSide = FXCollections.observableArrayList(arrSympValue[6], arrSympValue[7], arrSympValue[8]);
    private ObservableList<String> obsSleep = FXCollections.observableArrayList(arrTrig[4], arrTrig[5]);
    ...
    @FXML
    private Label lblAvgTrig;
    @FXML
    private Label lblAvgDuration;
    @FXML
    private Label lblMAvgSen;
    ...
    private void initialize() {
        ...
        Initialize.tabDiaryInitializeHd(tabTableColHd);
        Initialize.tabDiaryInitializeEp(tabTableColEp);
        ...
        if(lstHeadaches.isEmpty()){
            ...
            btnLatestSetting.setDisable(true);
            togglebtnAddHeadache.setSelected(true);
        }
        ...
        // EditMode
        EventHandler<MouseEvent> onClickEditMode = e -> onClickEditMode();
        btnModify.addEventFilter(MouseEvent.MOUSE_CLICKED, onClickEditMode);
    ...
    private void onClickAddInput() {
        ...
        if (!togglebtnAddEpisode.isSelected() && !togglebtnAddHeadache.isSelected()){
            ...
            if (togglebtnAddEpisode.isSelected()) {
                lastHeadache = controle.setEndDateHeadache(controle.getLastHeadache(), endDay);
    ...
    public void onClickEditMode() {
        boolean editMode = btnModify.isSelected();
        TableColumn[] tabTableColEp = {colIdEpisode, colDates, colSympCanvas, colSenCanvas, colTrigCanvas, colMed, colCom};
    ...
    protected void onClickFillLatestSettings() {
        ...
        List<String> sensibility = controle.getLstSensi(anEpisode);

        // Set values from Symptoms choiceBox
        ...
        if (triggers.contains(arrTrig[5])) {
            cobSleep.setValue(arrTrig[5]);
            chkSleep.setSelected(true);
        }
        else if(triggers.contains(arrTrig[4])){
            cobSleep.setValue(arrTrig[4]);
            chkSleep.setSelected(true);
        }
        if (triggers.contains(arrTrig[6])) {
        ...
        triggers = triggers.replaceAll("(?:Light|Sound|Smell|Movement|Period|Sleep|Lake of Sleep|[ \\[\\]])","")
                .replace(",","");
    ...
    public void loadDiary(){
        try {
            ...
            System.err.println("FXMLMyDiaryController : Erreur au chargement du journal XML headache");
            e.printStackTrace();
        }
    }
